...
namespace graphics_lib
{
	...
	public:
		...
		void SetColor(uint32_t rgbColor) override
		{
			...
			cout << "color #" << std::hex << uppercase << rgbColor << std::dec << endl;
...
namespace app
{
	...
	public:
		...
		void SetColor(uint32_t rgbColor) override
		{
			m_rgbaColor.r = ((rgbColor >> 16) & 0xff) / 255.f;  // Extract the RR byte
			m_rgbaColor.g = ((rgbColor >> 8) & 0xff) / 255.f;   // Extract the GG byte
